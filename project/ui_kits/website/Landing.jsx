...
const WINGS = [
  {
    key: "systems", num: "Wing 01", icon: "systems", title: "Business Systems",
  ...
  {
    key: "marketing", num: "Wing 02", icon: "megaphone", title: "Marketing",
    desc: "Demand, brand, and growth systems that feed the same connected operating company. Built on the same control layer.",
    status: "soon",
  },
  {
    key: "accounting", num: "Wing 03", icon: "calc", title: "Accounting Services",
    desc: "Books, compliance, and financial visibility wired directly into your systems — so the numbers are never a surprise.",
    status: "soon",
  },
];

function Landing({ go, onMind, onBook }) {
  return (
    <main data-screen-label="Landing">
      {/* HERO */}
      <section data-label="Hero" className="section grid-bg" style={{ paddingTop: 200, paddingBottom: 120, position: "relative", overflow: "hidden" }}>
        <canvas className="lh-network" data-density="0.7" data-formed="0.8" data-seed="11" data-speed="0.7" data-core="1"
        ...
        <img src="../../assets/logos/leanhippo-white.png" alt="" aria-hidden="true" className="hero-watermark"
          style={{ position: "absolute", left: "50%", top: "54%", transform: "translate(-50%,-50%)",
            width: "min(680px, 78%)", opacity: 0.05, zIndex: 1, pointerEvents: "none" }} />
        <div className="wrap" style={{ position: "relative", zIndex: 2, textAlign: "center" }}>
          ...
          <p className="lead" style={{ margin: "30px auto 0", maxWidth: "56ch" }}>
            Lean Hippo gives businesses control, visibility, and intelligence — so they stop running on memory and start running on systems.
          </p>
          ...
          <div style={{ display: "flex", gap: 12, justifyContent: "center", marginTop: 30, flexWrap: "wrap" }}>
            <span className="pill"><span className="dot" style={{ background: "var(--accent)", boxShadow: "0 0 8px var(--accent)" }} />Operational infrastructure</span>
        ...
        <div className="wrap">
          ...
          <div className="wings">
            {WINGS.map((w) => {
              ...
              return (
                <div key={w.key} className={"wing " + (live ? "available" : "soon")}
                  onClick={live ? () => go("systems") : () => onBook("bottleneck")}>
                  <div className="edge" />
                  <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center" }}>
                    <span className="wnum">{w.num}</span>
                    <span className={"pill " + (live ? "live" : "soon")}>
                      <span className="dot" />{live ? "Available now" : "Coming soon"}
                    </span>
                  </div>
                  <div className="wicon"><WIcon name={w.icon} size={26} color="var(--lh-cobalt-100)" /></div>
                  <div className="wtitle">{w.title}</div>
                  <div className="wdesc">{w.desc}</div>
                  <div className="wfoot">
                    {live ? (
                      <span className="btn btn-primary" style={{ pointerEvents: "none" }}>
                        Enter Business Systems <WIcon name="arrowUR" size={16} color="#fff" />
                      </span>
                    ) : (
                      <span className="btn btn-secondary" style={{ pointerEvents: "none" }}>
                        Join the waitlist <WIcon name="arrow" size={15} />
                      </span>
                    )}
                  </div>
                </div>
          ...
          {/* MIND+ strip */}
          <div className="glass lit" style={{ marginTop: 26, padding: "28px 34px", borderRadius: "var(--radius-lg)",
            ...
            <div style={{ flex: 1, minWidth: 240 }}>
              ...
              <p className="body" style={{ marginTop: 10, maxWidth: "62ch" }}>
                An add-on to every service so your system grows with you and learns from your everyday decisions — or a standalone product on its own.
              </p>
            </div>
            <button className="btn btn-secondary btn-lg" style={{ flex: "0 0 auto" }} onClick={onMind}>
              What is MIND+ <WIcon name="arrow" size={16} />
            </button>
          </div>
        </div>
      </section>
