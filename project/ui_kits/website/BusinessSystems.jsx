...
const ORGANS = [
  { fn: "01 · Visibility", icon: "gauge", title: "The Control Room", desc: "Your whole business — cash, sales, stock, tasks, approvals — live on one screen.",
  ...
  { fn: "02 · Money", icon: "vault", title: "The Vault", desc: "Full financial control — cash movement, approvals, monthly P&L, cash flow, and reporting. Nothing of value moves unseen.",
  ...
  { fn: "03 · Customers", icon: "switchboard", title: "The Switchboard", desc: "Every enquiry caught, answered, assigned, and tracked — across social, web, email, and calls.",
  ...
  { fn: "04 · Knowledge", icon: "book", title: "The Playbook", desc: "SOPs, checklists, and operating knowledge captured into a system your staff can actually follow.",
    inc: ["SOP library", "Staff checklists", "Onboarding flows", "Role instructions", "Escalation rules", "Searchable knowledge base"] },
  { fn: "05 · Procurement", icon: "truck", title: "The Supply Line", desc: "Supplier prices, requests, approvals, and reorders move through one controlled pipeline.",
  ...
  { fn: "06 · Decisions", icon: "brain", title: "The Mind", desc: "The AI intelligence layer that answers from your business's own operating memory.",
...
const WORKSWITH = ["Instagram", "Facebook", "POS", "Spreadsheets", "Accounting tools",
  "Forms", "Inventory records", "Supplier records", "Google Drive", "PDFs", "Email records"];
...
function BusinessSystems({ go, onMind, onBook }) {
  return (
    <main data-screen-label="Business Systems">
      ...
      <section data-label="BS Hero" className="section grid-bg" style={{ paddingTop: 180, paddingBottom: 90, position: "relative", overflow: "hidden" }}>
        <canvas className="lh-network" data-density="0.5" data-formed="0.9" data-seed="4" data-core="1" data-speed="0.7"
          style={{ position: "absolute", inset: 0, width: "100%", height: "100%", opacity: .45, zIndex: 0 }} />
        ...
        <div className="wrap" style={{ position: "relative", zIndex: 2 }}>
          ...
          <p className="lead" style={{ marginTop: 26, maxWidth: "54ch" }}>
            Not more scattered tools. Lean Hippo installs the control layer between your people, money, customers, stock, suppliers, approvals, and decisions.
          </p>
          <div style={{ display: "flex", gap: 14, marginTop: 38, flexWrap: "wrap" }}>
            <button className="btn btn-primary btn-lg" onClick={() => onBook("bottleneck")}>
              Book a Discovery Session <WIcon name="arrow" size={17} color="#fff" />
            </button>
            <button className="btn btn-secondary btn-lg" onClick={() => go("systems", "organs")}
      ...
      <section data-label="The Problem" className="section tight" style={{ paddingTop: 70, paddingBottom: 70 }}>
        <div className="wrap" style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 56, alignItems: "center" }}>
          ...
          <div>
            <p className="body">
              Information sits across spreadsheets, POS reports, people's heads, supplier chats, and scattered files. The business works — but only because people hold it together every day.
            </p>
          ...
          <div className="organs">
            {ORGANS.map((o, i) => (
              <div className="organ" key={i}>
                <div className="oicon"><WIcon name={o.icon} size={20} color="var(--lh-cobalt-100)" /></div>
                <div className="ofn">{o.fn}</div>
                <div className="otitle">{o.title}</div>
                <div className="odesc">{o.desc}</div>
                <ul className="inc-list">
                  {o.inc.map((x, k) => <li key={k}>{x}</li>)}
                </ul>
              </div>
            ))}
          </div>
      ...
      <section data-label="Vault Financials" className="section tight" id="financials"
        style={{ background: "radial-gradient(120% 100% at 15% 0%, #0B0F16, var(--lh-black))" }}>
        <div className="wrap">
          <div style={{ display: "flex", alignItems: "flex-end", justifyContent: "space-between", flexWrap: "wrap", gap: 24, marginBottom: 44 }}>
            <div>
              ...
              <p className="body" style={{ marginTop: 18, maxWidth: "46ch" }}>
                Beyond approvals and cash control, the Vault runs your reporting — monthly P&amp;L, cash flow, reconciliation, and the numbers you need to make decisions.
              </p>
            </div>
            <button className="btn btn-primary btn-lg" onClick={() => onBook("financial")}>
              Book a financial / P&amp;L review <WIcon name="arrow" size={16} color="#fff" />
            </button>
          </div>
          <div className="organs">
            {VAULT_FINANCIALS.map((f, i) => (
              <div className="organ" key={i}>
                <div className="oicon"><WIcon name={f.icon} size={20} color="var(--lh-cobalt-100)" /></div>
        ...
        <div className="wrap" style={{ display: "grid", gridTemplateColumns: "0.8fr 1.2fr", gap: 56, alignItems: "center" }}>
          <div>
            ...
            <p className="body" style={{ marginTop: 20, maxWidth: "40ch" }}>
              Four stacked layers, connected by signal lines. Not just automations — architecture that turns scattered work into one operating system.
            </p>
            <button className="btn btn-secondary" style={{ marginTop: 26 }} onClick={onMind}>
              <span className="mind-orb" style={{ width: 22, height: 22, animation: "none", flex: "0 0 auto" }}>
                <span className="glyph"><span className="m" style={{ fontSize: 11 }}>M</span><span className="p" style={{ fontSize: 9 }}>+</span></span>
              </span>
            ...
            {LAYERS.map((l, i) => (
              <React.Fragment key={i}>
                <div className={"layer" + (l.lit ? " lit" : "")}>
                  <div><div className="lk">{l.k}</div><div className="lname">{l.n}</div></div>
        ...
        <div className="wrap">
          <div style={{ display: "flex", alignItems: "flex-end", justifyContent: "space-between", flexWrap: "wrap", gap: 20, marginBottom: 40 }}>
            ...
            <p className="body" style={{ maxWidth: "40ch" }}>
              The system adapts to the business before the business is asked to adapt to the system — through integrations, structured imports, forms, and lightweight tools.
            </p>
          </div>
          <div className="chipgrid">
            {WORKSWITH.map((w, i) => <span className="chip2" key={i}><span className="dot" />{w}</span>)}
          </div>
        </div>
      </section>
      ...
      <section data-label="Human Approval" className="section tight">
        <div className="wrap">
          ...
          <p className="body" style={{ marginBottom: 44, maxWidth: "60ch" }}>
            Automation removes drudgery, not authority. The system can remind, draft, route, detect, and summarise — critical actions stay gated behind people.
          </p>
          <div className="steps">
            {SAFEGUARDS.map((s, i) => (
              <div className="step" key={i}>
                <div className="oicon" style={{ marginBottom: 16 }}><WIcon name={s.ic} size={20} color="var(--lh-cobalt-100)" /></div>
                <div className="sh">{s.h}</div>
                <div className="sd">{s.d}</div>
              </div>
            ))}
          ...
          <div className="changes">
            {CHANGES.map((c, i) => (
              <div className="change" key={i}>
                ...
                <WIcon name="arrow" size={18} color="var(--lh-cobalt-100)" />
          ...
          <div className="glass lit grid-bg" style={{ padding: "60px 56px", borderRadius: "var(--radius-xl)", position: "relative", overflow: "hidden" }}>
            ...
            <div style={{ position: "relative", zIndex: 2, maxWidth: 680 }}>
              ...
              <p className="lead" style={{ marginTop: 22, maxWidth: "50ch" }}>
                Start with a Bottleneck Discovery Session — a business scan that maps where control is leaking, where information is scattered, and which system to build first. MIND+ is included free to evaluate.
              </p>
              <div style={{ display: "flex", gap: 14, marginTop: 34, flexWrap: "wrap" }}>
                <button className="btn btn-primary btn-lg" onClick={() => onBook("bottleneck")}>
                  Book a Discovery Session <WIcon name="arrow" size={17} color="#fff" />
                </button>
                <button className="btn btn-ghost btn-lg" id="mindplus" onClick={onMind}>What is MIND+</button>
              </div>
            </div>
          </div>
        </div>
