...
/* ============================================================
   Node Network motif — the signature "business nervous system".
   Ported from the design bundle's network.js into a self-contained
   React canvas component. Honours prefers-reduced-motion.
   ============================================================ */

type NetworkProps = {
  density?: number; // 0..1 how many links light up
  formed?: number; // 0..1  0 = scattered/forming, 1 = connected
  seed?: number; // layout seed
  speed?: number; // signal speed multiplier
  core?: boolean; // draw a bright central AI core
  className?: string;
  style?: CSSProperties;
};
...
export function Network({
  ...
  useEffect(() => {
    ...
    let W = 0;
    let H = 0;
    type Node = { x: number; y: number; r: number; lit: boolean; pulse: number; core?: boolean };
    type Edge = { a: number; b: number; lit: boolean; phase: number; len: number };
    let nodes: Node[] = [];
    let edges: Edge[] = [];
    let raf = 0;
    ...
    function layout() {
      ...
      if (core) nodes.push({ x: cx, y: cy, r: 6, lit: true, core: true, pulse: 0 });
    ...
    function draw(t: number) {
      ...
      for (const nd of nodes) {
        const breathe = reduce ? 0.5 : 0.5 + 0.5 * Math.sin(t * 1.4 + nd.pulse * 6.28);
        if (nd.core) {
          const R = 9 + breathe * 4;
  ...
  }, [density, formed, seed, speed, core]);
